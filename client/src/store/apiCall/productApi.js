...
export const createReviewApi = async ({ data, setProduct, user }, resetForm) => {
...
export const toggleReviewLikeApi = async ({ revId, setProduct, user, reviews }) => {
  if (Object.keys(user).length === 0) return dispatch(setDialog(true))
  try {
...
export const getProductsApi = async (page) => {
  try {
    ...
    if (response?.data?.products.length <= 0) toast.error(`We don't have products in page ${page}`)
  } catch (e) {}
}
