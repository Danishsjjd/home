...
export const addToCartApi = async (user, product, cart, quantity) => {
  ...
  try {
    ...
    if (!search) bucket = [...cart, { productId: product, quantity: updatedCart.data.quantity }]
    else {
...
export const removeProductCartApi = async (cart, productId) => {
  try {
    ...
    const bucket = cart.filter((singleProduct) => singleProduct.productId._id !== productId)
    dispatch(setCart(bucket))
